...
안드로이드 개발자에게 가장 고민스러운 부분 중 하나는 100개 이상의 각기 다른 단말기의 화면 사이즈에 이미지를 어떻게 맞출 수 있을까이다. 이미지는 조금만 잘못 만져도 깨지고 픽셀이 보이기 일쑤다.

 따라서 안드로이드에서 사용되는 방식이 **나인패치(Nine Patch) 이미지**다. 나인패치는 이미지 사이즈 변동에도 깨지지 않게 보여주는 기능이다. 버튼 이미지처럼 같은 모양의 반복이나 일정 부분에 해당되며 일반 사진은 해당되지 않는다.

 나인패치를 적용하지 않은 버튼의 가로를 원본보다 길게 만든다면 네 개의 모서리가 깨져서 보기 흉해질 것이다. 하지만 그와 반대로 나인패치 이미지로 만들면 모서리가 깨지지 않고 부드럽게 연출된다.

 나인패치 만드는 방법은 버튼에 늘어나도 가능한 모서리의 겉에 검은색 선으로 한 칸 띄워 선을 그어줍니다. 그 후 파일 작성자명을 수정해줘야 하는데, 기존의 파일이름은 ```button1.png``` 이었다면 ```button1.9.png```로 수정해야 나인패치 이미지로 적용 가능하다.

 사실 나인패치 이미지는 개발자의 입장보단 디자이너의 입장에서의 역할이 크지만 전반적으로 이해하는 차원에서 기억하자.
- - -
