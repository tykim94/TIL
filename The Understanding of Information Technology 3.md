...
3. 모뎀 전화선
 - 주로 전화선을 이용한 모뎀 이용
 - 모뎀(Modem)은 컴퓨터에서 처리한 0과 1의 디지털 데이터와 아날로그 데이터를 변환시키는 Modulation(변조)과 디지털 데이터를 다시 아날로그로 변환하는 Demodulation (복조)의 과정을 통해 인터넷 연결
...
4. 온라인 서비스
 ...
 - 컴퓨터 생산업체들이 만들어 내는 다양한 하드웨어 플랫폼과 소프트웨어에서 운용될 수 있는 표준화된 연결고리인 Winsock(Windows Sockets) 프로그램으로 관련 생산업체가 동의한 표준화 된 인터페이스(Interface)
...
1. 인터넷으로 자료를 검색하거나 파일을 전송할 때 소요되는 시간은 각 컴퓨터와 인터넷 백본과의 연결된 매체의 대역폭(Bandwidth)과 직접적인 관련
...
3. 국내 인터넷 사용속도
 - 최근 FTTH(Fiber to The Home)의 보급으로 인터넷 사용속도가 Gbps급으로 성장
 - 2012년 대한민국- 17.5Mbps
 - 세계 인터넷속도- 2.3Mbps
 - 통신속도: bps(bits per second), 1byte=8bits, K:1000, M:1,000,000, G:1,000,000,000
...
1. 인트라넷
 ...
 - 인트라넷은 외부의 무단 접속을 차단하기 위해 방화벽(Firewall) 사용
